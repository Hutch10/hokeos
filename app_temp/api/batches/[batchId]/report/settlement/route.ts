...
import { generateSettlementSheet, logReportGeneration } from "@/lib/reports";
...
export async function GET(
  ...
  try {
    ...
    const report = await generateSettlementSheet(parsed.data.data);
    await logReportGeneration(user.id, user.activeTeamId, batchId, "settlement");

    return new Response(new Uint8Array(report.pdf), {
      status: 200,
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="settlement-sheet-${batchId}.pdf"`,
    ...
    console.error("[GET /api/batches/[batchId]/report/settlement] error:", err);
    return Response.json({ ok: false, error: "Failed to generate settlement sheet" }, { status: 500 });
  }
}
