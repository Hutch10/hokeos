...
import { generateMeltSheet, logReportGeneration } from "@/lib/reports";
...
export async function GET(
  ...
  try {
    ...
    const report = await generateMeltSheet(parsed.data.data);
    await logReportGeneration(user.id, user.activeTeamId, batchId, "melt");

    return new Response(new Uint8Array(report.pdf), {
      status: 200,
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="melt-sheet-${batchId}.pdf"`,
    ...
    console.error("[GET /api/batches/[batchId]/report/melt] error:", err);
    return Response.json({ ok: false, error: "Failed to generate melt sheet" }, { status: 500 });
  }
}
