import { buildComparisonCsv } from "@/lib/export/csv-builder";
...
export async function GET(req: Request) {
  try {
    ...
    const csv = buildComparisonCsv(batches);

    return new Response(csv, {
      status: 200,
      headers: {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="batch-comparison-${Date.now()}.csv"`,
    ...
    console.error("[GET /api/batches/compare/export/csv] error:", err);
    return Response.json({ ok: false, error: "Failed to export comparison CSV" }, { status: 500 });
  }
}
